...
public class FeatureVectorSequence implements Sequence {
    ...
    public FeatureVectorSequence(Alphabet dict, TokenSequence tokens,
        ...
        if (augmentable)
            ...
            for (int i = 0; i < tokens.size(); i++)
                sequence[i] = new AugmentableFeatureVector(dict,
                        tokens.getToken(i).getFeatures(),
                        binary,
                        growAlphabet);
        else

            for (int i = 0; i < tokens.size(); i++)
                sequence[i] = new FeatureVector(dict,
                        tokens.getToken(i).getFeatures(),
                        binary, growAlphabet);
    }
    ...
    public double dotProduct(int sequencePosition, Matrix2 weights,
                             int weightRowIndex) {

        return weights.rowDotProduct(weightRowIndex,
