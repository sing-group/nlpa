...
import es.uvigo.esei.pipe.PipeOutputAccumulator;
 ...
 * @since jdk1.5
 */
public class TokenSequence implements PipeOutputAccumulator, Sequence, Iterable/*jdk1.5*/ {
    ...
    public TokenSequence(int capacity) {
        this.tokens = new ArrayList<Token>(capacity);
    }
    ...
    public FeatureSequence toFeatureSequence(Alphabet dict) {
        ...
        for (int i = 0; i < tokens.size(); i++)
            fs.add(dict.lookupIndex(tokens.get(i).getText()));
