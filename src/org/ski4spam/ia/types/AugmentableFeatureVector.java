...
public class AugmentableFeatureVector extends FeatureVector {
     ...
     */
    public AugmentableFeatureVector(Alphabet dict, int[] indices,
                                    double[] values, int capacity, int size,
                                    boolean copy, boolean checkIndicesSorted,
                                    boolean removeDuplicates) {
        super(dict, indices, values, capacity, size, copy, checkIndicesSorted,
                removeDuplicates);
        this.size = size;
    ...
    public AugmentableFeatureVector(Alphabet dict, int[] indices,
                                    ...
                                    boolean copy, boolean checkIndicesSorted) {
        this(dict, indices, values, capacity, indices.length, copy,
                checkIndicesSorted, true);
    }
    ...
    public AugmentableFeatureVector(Alphabet dict, double[] values) {
        this(dict, null, values, values.length, values.length, true, true,
                true);
    }

    public AugmentableFeatureVector(Alphabet dict, int capacity,
                                    boolean binary) {
    ...
    public AugmentableFeatureVector(FeatureVector fv) {
        this(fv.dictionary, fv.indices, fv.values,
                ...
                fv.indices == null ? fv.values.length : fv.indices.length, true,
                false, false);
    }
    ...
    public AugmentableFeatureVector(Alphabet dict, PropertyList pl,
                                    boolean binary, boolean growAlphabet) {
        this(dict, binary);
    ...
    public ConstantMatrix cloneMatrix() {
        return new AugmentableFeatureVector(dictionary, indices,
                values, indices.length, size, true,
                false, false);
    }
    ...
    public ConstantMatrix cloneMatrixZeroed() {
        if (indices == null)
            ...
            System.arraycopy(indices, 0, newIndices, 0, indices.length);
            return new AugmentableFeatureVector(dictionary, newIndices,
                    new double[values.length],
    ...
    public int singleSize() {
        return (indices == null
                ? values.length : (size == 0 ? 0 : indices[size - 1]));
    }
    ...
    public SparseVector toSparseVector() {
        ...
        return new SparseVector(indices, values, size, size, true, false,
                false);
    }
    ...
    public FeatureVector toFeatureVector() {
        if (indices != null && size - 1 != maxSortedIndex)
            sortIndices();
        return new FeatureVector(dictionary, indices, values, size,
